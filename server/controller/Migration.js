...
module.exports = {
				...
				customers
					...
					.forEach(async (cus) => {
						...
						if ((status_info !== null || status_info === 'Customer added') && active && cust[0]) {
							...
							// let email_status = status !== 'Second Reminder' && status !== 'First Sent' ? 'open' : 'delivered';
							// FIX ABOVE
	...
	updateAPIs: async (req, res) => {
		let { e } = req.body;
		await req.app.get('db').migrate.update_api([e.c_id, e.c_api]);
		res.status(200).send({ msg: 'GOOD' });
	},
};
