...
let newReviews = (reviews, loc, rec) => {
	if (typeof rec === 'object' && typeof loc === 'number') {
		if (rec.reviews.length >= loc) {
			...
			rec.reviews[loc - 1].push({
				totalReviews: reviews,
				newReviews: reviews - last,
				date: today(),
			});
			return rec;
		} else {
		...
		let obj = {
			Standard: {
				...
				Body: {
					...
					thanks: standard.body.thanks,
					question: standard.body.question,
				},
			},
			First: {
			...
			Opened: {
				...
				Body: {
					...
					thanks: opened.body.thanks,
					question: opened.body.question,
				},
			},
			Positive: {
				subject: positive.subject,
				Body: {
					body: positive.body.body,
					thanks: positive.body.thanks,
					question: positive.body.question,
				},
			},
		};
		return obj;
...
let review = {
	Standard: {
		subject: 'Please Give Us A Review',
		Body: {
			body: 'Please For the love of pete give us a review',
			thanks: 'Thank You!',
		},
	},
	First: {
...
let review_landing = {
	...
	demoter: {
		thanks: 'Okay Mr. Bronze',
		body: "Bruh, You blow butt. We're awesome and you suck",
	},
};
let addon_landing = {
...
let settings = {
	...
	email_format: {
		...
		open: 2,
		positive: 1,
		logo: process.env.REACT_APP_SITE_LOGO,
		color: 'c4040c',
		frequency: 30,
		from_email: `no-reply@${process.env.REACT_APP_COMPANY_EXTENSION}.com`,
	},
	process: 1,
	repeat: 180,
	first: 2,
...
let accountManager = oid => {
	...
	if (oid === '0056A000002o98TQAQ') {
		...
		return { name: `Chris White`, email: `cwhite@${process.env.REACT_APP_COMPANY_EXTENSION}.com` };
	} else if (oid === `0056A000002W45GQAS`) {
		return { name: `Dylan Bakker`, email: `dbakker@${process.env.REACT_APP_COMPANY_EXTENSION}.com` };
	} else if (oid === `0053s0000034S2ZAAU`) {
		return { name: `Dylan Payne`, email: `dpayne@${process.env.REACT_APP_COMPANY_EXTENSION}.com` };
	} else if (oid === `0053s000002lRoyAAE`) {
		return { name: `Kodi Porter`, email: `kporter@${process.env.REACT_APP_COMPANY_EXTENSION}.com` };
	} else {
		return { name: `Ryan H`, email: `rhutchison@${process.env.REACT_APP_COMPANY_EXTENSION}.com` };
	}
};
// Exports
module.exports = {
