...
module.exports = {
	...
	siteHTML: async (req, res) => {
		...
		if (result.status === 200) {
			...
			res.status(200).send({ msg: 'There was an error in finding the link. Try again in a few minutes.' });
		}
		// request(url, function(error, response, html) {
		// 	if (!error && response.statusCode == 200) {
		// 		// let results = $('a').map(item => {
		// 		// 	console.log(item);
		// 		// 	// return item.split('">')[0].replace(/ /g, '+');
		// 		// });
		//
		// 	} else {
		// 		res.status(200).send({ msg: 'There was an error in trying to find facebook page', error: { error, response, html } });
		// 	}
		// });
	},
};
