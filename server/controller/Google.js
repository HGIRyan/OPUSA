...
module.exports = {
	...
	posts: async (req, res) => {
		try {
			let { accessToken, locations, imgLink, articleLink, postText } = req.body;
			let posting = async ({ location_id, i }) => {
				console.log(i);
				await axios
					.post(
						`https://mybusiness.googleapis.com/v4/accounts/${AccountID}/locations/${location_id}/localPosts`,
						{
							languageCode: 'en-US',
							summary: postText,
							callToAction: {
			...
			for (let i = 0; i < locations.length; i++) {
				const { location_id, location_name } = locations[i];
				setTimeout(async () => {
					await posting({ location_id, i });
					await console.log(AccountID, location_id, location_name);
					if (i === locations.length - 1) {
						await res.status(200).send({ msg: 'GOOD' });
					}
				}, i * 2500);
			}
		} catch (error) {
