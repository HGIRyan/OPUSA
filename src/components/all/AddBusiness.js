		...
		await axios
			...
			.then(res => {
				if (res.data.msg === 'GOOD') {
					...
					let street =
						address_components.filter(e => e.types.some(el => el === 'street_number'))[0].short_name +
						' ' +
						address_components.filter(e => e.types.some(el => el === 'route'))[0].long_name;
					...
					let city = address_components.filter(e => e.types.some(el => el === 'locality'))[0].long_name;
							...
							<div style={{ display: 'flex', width: '100%', height: '100%', border: 'solid black 1px' }} className="card-content">
								{page === 1 ? (
													...
													{this.state.formData.file ? (
														...
														<div onClick={() => this.getUploaded()}>
															<Modal
																...
																style={{ outline: 'none', minHeight: !this.state.imgLoaded ? '30vh' : 'auto' }}
																bottomSheet={false}
																fixedFooter={false}
																				...
																				? this.state.images.map((e, i) => {
																						return (
																							<img
																								...
																								key={i}
																								onClick={() => {
																									this.setState({ img: e.secure_url, images: {}, logo: e.secure_url, modalOpen: false });
																								}}
																							/>
																						);
																				  })
									...
									<div style={{ display: 'flex', flexDirection: 'column', width: '90%', justifyContent: 'flex-start' }}>
										...
										<hr />
										<div style={{ width: '50%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '5%' }}>
											<div className="input-field" style={{ height: '5vh', width: '60%' }}>
												<h2 style={{ margin: '0' }}>
													<input value={this.state.sf_api} onChange={e => this.setState({ sf_key: e.target.value })} />
												</h2>
												<label>SF API</label>
											</div>
