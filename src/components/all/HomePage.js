...
class HomePage extends Component {
	...
	async Search(value) {
		...
		if (value.length >= 2 && value.length % 2 === 0) {
			...
			similar = await og
				.filter(
					item =>
						simString.compareTwoStrings(value, item.company_name.toLowerCase()) >= 0.4 ||
	...
	handleCompanyClick(totLoc, pos, comp) {
		...
		if ((!cor_id && pos === 1 && totLoc.length === 1) || comp.searched) {
			...
			this.props.history.push(`/home/${totLoc[0].cor_id}`, this.props.location.state);
		} else {
			this.props.history.push(`/client-dash/${totLoc[pos - 1].cor_id}/${totLoc[pos - 1].c_id}`, this.props.location.state);
		}
	}
	async CompanyInfo(info, comp) {
		let { cor_id } = this.props.match.params;
		if (!cor_id || comp.searched) {
			info = info
				.sort((a, b) => (a.c_id > b.c_id ? 1 : -1))
				.sort((a, b) => (a.cor_id < b.cor_id ? 1 : -1))
				.sort((a, b) => (a.active < b.active ? 1 : -1));
		} else {
			info = info.sort((a, b) => (a.c_id > b.c_id ? 1 : -1)).filter(e => e.cor_id === parseInt(cor_id));
		}
		let { current, perPage } = this.state;
		let page = pagination(info, current, perPage);
		// let unique = info.filter((value, index, self) => self.map(x => x.cor_id).indexOf(value.cor_id) === index)
		if (!comp.searched && !cor_id) {
			...
			if (cor_id || pos === 1 || comp.searched) {
				return (
					<div
						...
						key={i}
						style={{
							border: `${total === 0 || !e.active ? 'solid rgba(250, 0, 0, .9) 5px' : remaining === 0 ? 'solid rgba(250, 0, 0, .5) 5px' : ''}`,
							margin: '1vh 0',
					...
					>
						<div className="card-content center-align" style={{ cursor: 'pointer', display: 'flex', height: '5vh', padding: '0 2.5%' }}>
							...
							onClick={() => this.props.history.push(`/client-dash/${e.cor_id}/${e.c_id}`, this.props.location.state)}
						>
							<h6 style={{ margin: '0' }}>c_id: {e.c_id}</h6>
							<h6 style={{ margin: '0' }}>cor_id: {e.cor_id}</h6>
						</div> */}
					...
					<div className="navbar-fixed" style={{ position: '-webkit-sticky', top: '-8vh' }}>
						<nav className="tertiary-color" style={{ width: '30%', right: `${width >= 1500 ? '10vw' : '8vw'}`, boxShadow: 'none', height: '4vh' }}>
							<div className="nav-wrapper row" style={{ display: 'flex', alignItems: 'center' }}>
								...
								<div className="col s4 right" style={{ height: '90%', marginTop: width >= 1500 ? '-2.5vh' : '-4vh' }}>
									<StyledLink
