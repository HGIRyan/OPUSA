...
import { Layout1, LoadingWrapperSmall } from '../../../utilities/index';
...
class GMBDetails extends Component {
			...
			await axios.post(`/api/gmb/allaccounts`, { accessToken, api, search }).then(res => {
				...
				if (res.data.msg === 'GOOD' && !api) {
					...
					res.data.info.locations.map(e =>
						info.push({
							location_id: e.name.split('/')[3],
							location_name: e.locationName,
							verified: e.locationState.isVerified,
	...
	async allAverage() {
		...
		// prettier-ignore
		let one = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(17, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let two = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(16, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let three = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(15, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let four = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(14, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let five = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(13, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let six = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(12, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let sev = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(11, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let eig = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(10, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let nine = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(9, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let ten = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(8, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let elev = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(7, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let twel = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(6, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let thirt = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(5, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let fourt = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(4, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let fift = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(3, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let sixt = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(2, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
		let sevent = { website: 0, calls: 0, direct: 0, indirect: 0, chain: 0, views: 0, total: 0, startTime: Moment().subtract(1, 'months').date(1).format('YYYY-MM-DD'), count: 0 };
		// prettier-ignore
	...
	async getLastMonth() {
		// get all accounts new data for last month
		alert('Please wait up to 5 - 10 minutes until refresh');
		let res = await axios.post('/api/gmb/pastmonth', { token: this.state.accessToken }, { timeout: 250 * 500 });
		if (res.data.msg === 'GOOD') {
			alert('Got New Insights for Last Month');
		} else {
				...
				<div style={{ minHeight: '50vh', width: '90%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
					{user === 'rhutchison' ? (
						<div style={{ width: '100%', height: '10vh', display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
							...
							<button className="btn primary-color primary-hover" style={{ marginLeft: '5%', marginTop: '2.5%' }} onClick={() => this.getLastMonth()}>
								Update Last Month
							</button>
						</div>
					) : null}
