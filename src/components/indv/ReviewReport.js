...
class ReviewReport extends Component {
	...
	async settingState({ exists, change, customDates, startDate }) {
		...
		let responses = info.filter(e => e.rating && e.last_sent !== '2005-05-25').length;
		let length = info.length;
		let open = info.filter(e => e.opened_time !== null && e.last_sent !== '2005-05-25').length;
		let sent = info.filter(e => e.last_sent !== '2005-05-25').length;
		let click = info.filter(e => e.click && e.last_sent !== '2005-05-25').length;
		let llRating = 0;
