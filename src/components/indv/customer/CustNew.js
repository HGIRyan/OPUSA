...
class CustNew extends Component {
	...
	render() {
		let { client_id, cor_id } = this.props.match.params;
		let data = { c_id: client_id, owner_name: { first: 'Boi, Unknown' }, company_name: process.env.REACT_APP_COMPANY_NAME };
		let { og } = this.state;
							...
							<NoDiv width="100%" height="45vh">
								<NoDiv width="50%" direction="column" padding="2.5%" height="100%">
									...
									<NoDiv just="center" align="center" height="2.5%">
										<label>
											<input type="checkbox" onChange={() => this.setState({ send: !this.state.send })} value={this.state.send} />
