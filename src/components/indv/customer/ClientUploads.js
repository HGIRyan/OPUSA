...
class ClientUpload extends Component {
	...
	async splitList() {
		...
		this.setState({ splitLoading: true });
		let loc = info.filter(e => e.cor_id === parseInt(cor_id));
		alert('This Process Will Take 20 Seconds to a Minute, Have Patience Young One');
					...
					<LoadingWrapper loading={this.state.loading}>
						{!data[0] && length > 1 && permission === 'admin' ? (
							<div style={{ width: '60vw', display: 'flex', justifyContent: 'flex-end' }}>
								<LoadingWrapperSmall loading={this.state.splitLoading}>
									<button className="btn primary-color primary-hover" onClick={() => this.splitList()}>
										Split List
									</button>
								</LoadingWrapperSmall>
							</div>
												...
												<Select value={this.state.uploadTo.toString()} onChange={e => this.setState({ uploadTo: e.target.value.toString() })}>
													{this.state.corp.map((e, i) => (
														<option value={e.c_id.toString()} key={i}>
															{`${e.c_id.toString()} - ${e.company_name.length >= 25 ? e.company_name.substring(0, 25) + '...' : e.company_name} - ${
																e?.address?.city
															}`}
														</option>
													))}
