					...
					<LoadingWrapper loading={this.state.loading}>
						...
						<div style={{ display: 'flex', justifyContent: 'space-between', width: '70vw' }}>
							<div style={{ width: '45%', marginRIght: '5%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
								...
								<h4>Accent Colors</h4>
								<blockquote className="left-align">
									The accent color is used in email designs and landing page designs. We have detected the colors below from your logo uploaded.
									{this.props.location.state.permissions === 'admin'
										? `Click on one of these colors to use it as your accent color, click the selection box for a small sample of base colors or enter a color's Hex number in the
									...
									{perm === 'admin' ? (
										...
										<ColorExtractor getColors={this.getColors}>
											<img
												src={logo}
												alt="COMPANY NAME"
												style={{ width: 'auto', maxWidth: '400px', height: 'auto', maxHeight: '400px', marginBottom: '-10vh' }}
											/>
										</ColorExtractor>
									)}
									{this.state.formData.file ? (
										...
										<div onClick={() => this.getUploaded()}>
											<Modal
												...
												style={{ outline: 'none', minHeight: !this.state.imgLoaded ? '40vh' : 'auto' }}
												bottomSheet={false}
												fixedFooter={false}
											>
												<div
																...
																this.state.images.map((e, i) => {
																	return (
																		<img
																			...
																			key={i}
																			onClick={() => {
																				this.setState({ link: e.secure_url, images: {}, logo: e.secure_url });
																			}}
																		/>
																	);
																})
