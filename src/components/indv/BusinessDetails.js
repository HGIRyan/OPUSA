...
class BusinessDetails extends Component {
	...
	async updateLogin() {
		let { newEmail, username, password, passCheck, og, permissionLevel } = this.state;
		let { c_id } = og;
		if (passCheck === password) {
			if (password.length >= 6) {
				password = cryptr.encrypt(password);
				await axios.post('/api/add-new-user/dev', { state: { email: newEmail, userName: username, password, cor_id: c_id, permissionLevel } }).then(res => {
					if (res.data.msg === 'GOOD') {
						window.location.reload();
					} else {
						alert(`Error: ${res.data.msg}`);
			...
			alert('Password And Password Check is Not The Same Value');
		}
	}
	async syncHGatherup() {
		let { og } = this.state;
