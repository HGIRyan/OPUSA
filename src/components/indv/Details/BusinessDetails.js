...
class BusinessDetails extends Component {
	...
	async updateLogin() {
		let { newEmail, username, og, permissionLevel } = this.state;
		this.setState({ saving: true });
		let { c_id } = og;
		if (newEmail) {
			await axios.post('/api/add-new-user/dev', { state: { email: newEmail, userName: username, cor_id: c_id, permissionLevel } }).then((res) => {
				if (res.data.msg === 'GOOD') {
					window.location.reload();
				} else {
					alert(`Error: ${res.data.msg}`);
	...
	async handleUserChange(e, type, val) {
		let { users } = this.state;
		let newObj = users.filter((el) => el.user_id === e.user_id)[0];
		if (type === 'sub') {
		...
		users.splice(
			users.findIndex((el) => el.user_id === e.user_id),
			1,
			newObj,
		);
		this.setState({ users });
	...
	async handleUserUpdate(e) {
		...
		if (e.password) {
			if (e.password.length >= 6) {
				...
				e.password = cryptr.encrypt(e.password);
				send(e);
			}
		} else {
			send(e);
			...
			<>
				<Layout1 view={{ sect: 'indv', data: this.state.og, loc }} match={this.props.match ? this.props.match.params : null} props={this.props}>
					<LoadingWrapper loading={this.state.loading}>
						{!this.state.dev && !this.state.userEdit && !this.state.viewUsers ? (
							<>
								<NoDiv just="flex-start" width="50%" align="center">
									<h3
										style={{ right: '30%', position: 'relative', cursor: edit ? 'pointer' : null }}
										onClick={() => (edit && permission === 'admin' && process.env.REACT_APP_SF_SECURITY_TOKEN ? this.setState({ dev: !this.state.dev }) : null)}
										className="noselect"
									>
										Business Details
									</h3>
								...
								{permission === 'admin' ? (
									<div style={{ width: '65vw', display: 'flex', alignItems: 'center', justifyContent: 'flex-start', margin: '0', marginTop: '-4.5vh' }}>
										<div style={{ display: 'flex', justifyContent: 'space-between', width: '40%' }}>
											<button className="btn primary-color primary-hover" onClick={() => this.setState({ userEdit: true })}>
												Add New User
											</button>
											<button
									...
									<div style={{ display: 'flex', flexDirection: 'column', width: '45%', marginLeft: '5%', alignItems: 'center' }}>
										...
										<div style={{ display: 'flex', width: '70%', justifyContent: 'space-between' }}>
											...
											<div className="input-field">
												<h2 style={{ margin: '0' }}>
													<input
														value={this.state.ownerLast}
														onChange={(e) => this.setState({ ownerLast: e.target.value })}
														disabled={edit ? '' : 'disabled'}
													/>
												</h2>
							...
							<>
								...
								>
									{this.state.users[0]
										? this.state.users.filter((e) => e.email !== `no-reply@${process.env.REACT_APP_COMPANY_EXTENSION}.com`)[0]
											? this.state.users
														...
														return (
															...
															>
																<div className="input-field" style={{ width: '10%' }}>
																	<h2 style={{ margin: '0' }}>
																		<input
																			...
																			className="validate"
																			value={e.c_id}
																			onChange={(el) => this.handleUserChange(e, 'c_id', el.target.value)}
																		/>
																	</h2>
																...
																<div className="input-field" style={{ width: '20%' }}>
																	<h2 style={{ margin: '0' }}>
																		<input
																			...
																			className="validate"
																			value={e.email}
																			onChange={(el) => this.handleUserChange(e, 'email', el.target.value)}
																		/>
																	</h2>
																...
																<div className="input-field" style={{ width: '20%' }}>
																	<h2 style={{ margin: '0' }}>
																		<input
																			...
																			className="validate"
																			value={e.username}
																			onChange={(el) => this.handleUserChange(e, 'username', el.target.value)}
																		/>
																	</h2>
																...
																<div className="input-field" style={{ width: '20%' }}>
																	<h2 style={{ margin: '0' }}>
																		<input
																			...
																			className="validate"
																			value={e.password}
																			onChange={(el) => this.handleUserChange(e, 'password', el.target.value)}
																		/>
																	</h2>
																...
																<div className="input-field" style={{ width: '10%', marginTop: '2%' }}>
																	...
																	<Select
																		name="View"
																		onChange={(el) => this.handleUserChange(e, 'sub', el.target.value)}
																		value={e.sub_perm.view_all.toString().toLowerCase()}
																	>
																		<option value="true">All Locations</option>
																		<option value="false">One Location</option>
																	</Select>
																</div>
																<button className="btn primary-color primary-hover" onClick={() => this.handleUserUpdate(e)}>
																	Update
																</button>
															</div>
														);
							...
							<>
								<NoDiv just="flex-start" width="50%" align="center">
									...
									<NoDiv width="40%">
										...
										<LoadingWrapperSmall loading={this.state.saving}>
											<button className="btn primary-color primary-hover" onClick={() => this.updateLogin()}>
												Create User
											</button>
										</LoadingWrapperSmall>
									</NoDiv>
									...
									<div className="input-field" style={{ width: '20vw', padding: '0', margin: '0' }}>
										<h2 style={{ margin: '0' }}>
											<input
												...
												onChange={(e) => {
													this.setState({ newEmail: e.target.value, username: e.target.value.split('@')[0] });
												}}
											/>
										</h2>
										<label htmlFor="email">Email: </label>
										...
										<div className="input-field" style={{ width: '70%' }}>
											<h2 style={{ margin: '0' }}>
												<input
													...
													type="text"
													className="validate"
													value={this.state.sf_id}
													onChange={(e) => this.setState({ sf_id: e.target.value })}
												/>
											</h2>
